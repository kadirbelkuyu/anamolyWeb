...
if(! function_exists('_get_current_user_companyname')){
    function _get_current_user_companyname(){
        $thi =& get_instance();
        return $thi->session->userdata("user_company_name");
    }
}
if(! function_exists('_get_current_user_image')){
    function _get_current_user_image(){
        ...
        if($thi->session->userdata("user_image") != "")
            return REMOTE_URL."/uploads/profile/".$thi->session->userdata("user_image");
        else
            return base_url(WEB_THEME_BASE."/images/avatar.png");
    }
}
