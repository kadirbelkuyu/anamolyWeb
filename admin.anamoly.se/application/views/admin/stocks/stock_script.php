<script>
        $("#offer_type").change(function(){
            ...
            if(val == "plusone" || val == "flatcombo"){
                ...
                if(val == "plusone"){
                    ...
                    $(".div_offer_discount").addClass("hide");
                    ...
                    $(".div_offer_discount").removeClass("hide");
                }
        ...
        $("#number_of_products").on("change paste keyup", function() {
            var val = $("#offer_type").val();
            if(val == "plusone"){
                var no_of_products = $(this).val();
                var discount = 0;
                if(no_of_products > 0){
                    discount = 100 / (no_of_products + 1);
                }
    ...
    $.ajax({
            type: "POST",
            url: '<?php echo site_url("admin/offers/change_status"); ?>',
            ...
            {   
                if(data.responce){
                    ...
                    if(prod.status == 1){
                        datatable.cell( row, 4 ).data( '<a href="javascript:changeStatus(\''+0+'\',\''+prod.product_offer_id+'\',\''+row+'\')" id="ref_'+prod.product_offer_id+'"><span class="label label-success"><?php echo _l("Published"); ?></span></a>' );
                    }else{
                        datatable.cell( row, 4 ).data( '<a href="javascript:changeStatus(\''+1+'\',\''+prod.product_offer_id+'\',\''+row+'\')" id="ref_'+prod.product_offer_id+'"><span class="label label-default"><?php echo _l("Pending"); ?></span></a>' );
                    }
                }
            }
            });
