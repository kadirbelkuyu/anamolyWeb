<?php class Vats_model extends CI_Model{
    ...
    function get($filter = array(),$search = "",$offcet="",$limit=""){
        ...
        if($search != ""){
            $this->db->or_like(array($this->table_name.'.vat_name'=>$search,
                                    $this->table_name.'.vat'=>$search));
        }
		$this->db->select("{$this->table_name}.*");
        $this->db->where($this->table_name.".draft","0");
		$this->db->order_by($this->table_name.".short_order");
        if($offcet !="" && $limit != ""){
