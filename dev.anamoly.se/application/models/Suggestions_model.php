<?php class Suggestions_model extends CI_Model{
    ...
    function get($filter = array(),$search = "",$offcet="",$limit=""){
        ...
        if($search != ""){
            $this->db->or_like(array($this->table_name.'.suggestion'=>$search));
        }
		$this->db->select("{$this->table_name}.*,users.user_firstname,users.user_lastname");
        $this->db->where($this->table_name.".draft","0");
        $this->db->join("users","users.user_id = ".$this->table_name.".user_id","left");
