<?php class Productgroups_model extends CI_Model{
    ...
    function get($filter = array(),$search = "",$offcet="",$limit=""){
        ...
        if($search != ""){
            $this->db->or_like(array($this->table_name.'.group_name_en'=>$search,
                                    $this->table_name.'.group_name_ar'=>$search,
                                    $this->table_name.'.group_name_nl'=>$search));
        }
		$this->db->select("{$this->table_name}.*,categories.cat_name_en,categories.cat_name_ar,categories.cat_name_nl
        ,sub_categories.sub_cat_name_en,sub_categories.sub_cat_name_ar,sub_categories.sub_cat_name_nl");
        $this->db->join("categories","categories.category_id = ".$this->table_name.".category_id","left");
        $this->db->join("sub_categories","sub_categories.sub_category_id = ".$this->table_name.".sub_category_id","left");
