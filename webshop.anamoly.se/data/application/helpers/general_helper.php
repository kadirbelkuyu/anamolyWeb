...
if (!function_exists('_back_url')) {
    function _back_url()
    {
        $CI =& get_instance();
        $previous = $CI->session->tempdata("back_url");
...
if(!function_exists('_set_back_url')){
    function _set_back_url(){
        $CI =& get_instance();
        $url = current_full_url();
        $CI->session->set_tempdata(array("back_url"=>$url), null, 3000);
    }
}
function _set_data_filter($key,$value=""){
    $CI =& get_instance();
    if(is_array($value)){
        foreach($value as $k=>$val){
            $k1 = $key."_".$k;
            $CI->session->set_tempdata(array($k1=>$val), NULL, 3000);
        }
    }else{
        $CI->session->set_tempdata(array($key=>$value), NULL, 3000);
    }
}
function _get_data_filter($key,$sub_key=""){
    $CI =& get_instance();
    if($sub_key != ""){
        $key = $key."_".$sub_key;
    }
...
function _toastmessage($message,$type="error"){

    return '<script type="text/javascript">toastScript("'.str_replace("\n","",$message).'","'.$type.'");</script>';

}
function set_flash_alert($message)
{
    $CI =& get_instance();
...
function get_flash_alert(){
    $CI =& get_instance();
    $message = $CI->session->flashdata('debug');
    if($message != NULL && $message != ""){
        _toastmessage($message);
    }
}

function _limage($image,$type,$default=""){
    if($image != ""){
        $path = REMOTE_URL."/".$type;
        $file = $path."/".$image;
        //if(file_exists($path."/".$image)){
        //    return base_url($type."/".$image);
        //}
    ...
    if($default != ""){
      ...
      return base_url("themes/webshop/images/placeholder.png");
    }
}
function _lname($obj,$name){
    $name = str_replace("_en","",$name);
    $name = str_replace("_nl","",$name);
    $name = str_replace("_tr","",$name);
    $name = str_replace("_ar","",$name);
    $name = str_replace("_de","",$name);
    $CI =& get_instance();
...
function _lprice($price){
    $currency = _currency_symbol();
    $price = sprintf('%0.2f', round($price, 2));
    $c = explode(".",$price);
    $p = $c[0];
    if(count($c) > 1){
      if($c[1] > 0){
